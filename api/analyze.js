...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isShort) {
      ...
      conflictResolutionReasonDesc = "1 sentence highlighting how naturally and warmly they coordinate on the face of a tough situation";
      ...
      //
      summaryDesc = `A friendly, comforting overview summary capturing the overall alignment. Synthesize their individual communication styles, how their personal habits complement or differ, and what simple adjustments they can make to align their rhythm gracefully over time.`;
      ...
      - Discuss how their personal styles interact (e.g. ${names.consistentPartner} being protective/having each other's back vs ${names.asyncPartner} reciprocating warmly).
      `;
    ...
    const makeStrategistPrompt = (personaData, dynamicsData) => {
      return `You are a behavioral strategist acting as a supportive relationship counselor. Review these profile files generated by the analysis agents:
      Individual Profiles: ${JSON.stringify(personaData)}
      Macro Dynamics: ${JSON.stringify(dynamicsData)}
      
      Write exactly 2 actionable advice points for each person using clear, comforting, everyday language. These must serve as a professional counselor's advice tailored specifically for both individuals based on a deep understanding of their chat and communication behavior. Do not show numbers.
